...
namespace LmCorbieUI.LmForms {
  ...
  public partial class LmSingleForm : Form, IDisposable {
    ...
    protected override void WndProc(ref Message m) {
      ...
      // Intercepta a mensagem que inicia o movimento do formulário
      if (m.Msg == WM_NCLBUTTONDOWN && !Movimentar) {
        return;  // Ignora a movimentação
      }
      base.WndProc(ref m);
    }
    ...
    private void AddWindowButton(WindowButtons button) {
      ...
      if (button == WindowButtons.Close) {
        ...
        this.toolTip1.SetToolTip(newButton, "Ajuda");
        newButton.Font = fntWeb;
        newButton.Text = "s";
